...
class HistogramColorClassifier:
    ...
    def returnHistogramComparison(self, hist_1, hist_2, method='intersection'):
        ...
        if cv2.__version__.split(".")[0] == '3':
            ...
            if(method=="intersection"):
                comparison = cv2.compareHist(hist_1, hist_2, cv2.CV_COMP_INTERSECT)
            elif(method=="correlation"):
                comparison = cv2.compareHist(hist_1, hist_2, cv2.CV_COMP_CORREL)
            elif(method=="chisqr"):
                comparison = cv2.compareHist(hist_1, hist_2, cv2.CV_COMP_CHISQR)
            elif(method=="bhattacharyya"):
                comparison = cv2.compareHist(hist_1, hist_2, cv2.CV_COMP_BHATTACHARYYA)
            else:
                raise ValueError('[DEEPGAZE] color_classification.py: the method specified ' + str(method) + ' is not supported.')
        return comparison
